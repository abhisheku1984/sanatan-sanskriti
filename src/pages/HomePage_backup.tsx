    ...
    <div className="min-h-screen">
      ...
      <section className="relative bg-cosmic-dark py-16 md:py-24 overflow-hidden">
        ...
        <div className="relative z-10 max-w-3xl mx-auto px-4 text-center">
          ...
          <motion.div initial={{ opacity: 0 }} whileInView={{ opacity: 1 }} transition={{ duration: 1, delay: 0.8 }} viewport={{ once: true }} className="mt-6">
            <p className="font-devanagari text-cosmic-gold/50 text-sm">कालोऽस्मि लोकक्षयकृत्प्रवृद्धः</p>
        ...
        <div className="max-w-4xl mx-auto px-4 grid grid-cols-3 gap-6">
          {[{ num: stats.books, label: t('sacredTexts', language) }, { num: stats.temples, label: t('sacredSites', language) }, { num: 12, label: t('languages', language) }].map((stat, i) => (
            <motion.div key={stat.label} initial={{ opacity: 0, y: 12 }} whileInView={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.1 }} viewport={{ once: true }} className="text-center">
              <p className="font-display text-3xl md:text-4xl font-bold text-vermillion">{loading ? '—' : stat.num}</p>
      ...
      <section className="py-12 px-4">
        <div className="max-w-3xl mx-auto">
          <div className="bg-gradient-to-br from-cosmic-dark to-cosmic-navy rounded-2xl p-8 text-center border border-cosmic-gold/20">
            <div className="flex justify-center mb-4"><DharmaWheel size={48} spinning={true} color="#C9A84C" /></div>
            <h2 className="font-display text-2xl font-bold text-cosmic-glow mb-2">{t('downloadSource', language)}</h2>
            <p className="text-cosmic-glow/50 text-sm mb-6 max-w-md mx-auto">{t('downloadDesc', language)}</p>
            <a href="https://auwgmpuohewmlazmlcun.supabase.co/storage/v1/object/public/downloads/sanatan-sanskriti-source.zip" download="sanatan-sanskriti-source.zip" target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-2.5 px-7 py-3 rounded-xl bg-cosmic-gold/20 border border-cosmic-gold/40 text-cosmic-glow font-semibold text-sm hover:bg-cosmic-gold/30 transition-colors"><Download size={18} /> {t('downloadBtn', language)}</a>
            <p className="text-cosmic-glow/30 text-[11px] mt-3">{t('downloadNote', language)}</p>
          </div>
        </div>
      </section>
      ...
      <section className="py-12 px-4">
        <div className="max-w-3xl mx-auto">
          <h2 className="font-display text-2xl font-bold text-ink text-center mb-6">{t('addingBooks', language)}</h2>
          <div className="bg-surface border border-border rounded-xl p-6">
            <p className="text-ink-muted text-sm mb-4">{t('addBooksDesc', language)}</p>
            <div className="space-y-3">
              {(['step1','step2','step3','step4'] as const).map((key, i) => (
                <div key={key} className="flex items-start gap-3">
                  <span className="w-6 h-6 rounded-full bg-vermillion/10 text-vermillion text-[12px] font-bold flex items-center justify-center flex-shrink-0 mt-0.5">{i+1}</span>
                  <p className="text-ink-light text-sm">{t(key, language)}</p>
                </div>
              ))}
            </div>
          </div>
      ...
      <footer className="relative bg-cosmic-dark text-cosmic-glow/70 py-12 px-4 mt-8">
        <div className="relative z-10 max-w-5xl mx-auto text-center">
          ...
          <p className="font-devanagari text-cosmic-gold/50 text-xs mt-2 samay-fade">॥ मैं समय हूँ — कालोऽस्मि लोकक्षयकृत्प्रवृद्धोऽहम् ॥</p>
          <div className="my-4 flex items-center justify-center gap-3"><div className="h-px w-16 bg-cosmic-gold/20" /><span className="text-cosmic-gold/40 text-xs">✦</span><div className="h-px w-16 bg-cosmic-gold/20" /></div>
