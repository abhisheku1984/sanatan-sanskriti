...
export default function LibraryPage({ language }: { language: string }) {
    ...
    fetch('/api/books')
      ...
      .then(data => {
        if (Array.isArray(data)) {
          ...
          console.warn('Books API returned non-array:', data);
          setBooks([]);
  ...
  if (selectedBook) return <div className="pt-6 pb-12 px-4"><BookReader book={selectedBook} onBack={() => setSelectedBook(null)} language={language} /></div>;
      ...
      <div className="max-w-5xl mx-auto">
        ...
        <div className="flex flex-col sm:flex-row gap-3 mb-6">
          ...
          <div className="flex items-center border border-border rounded-lg overflow-hidden">
            <button onClick={() => setView('grid')} className={`px-3 py-2.5 ${view === 'grid' ? 'bg-vermillion/10 text-vermillion' : 'text-ink-muted'}`}><Grid size={15} /></button>
            <button onClick={() => setView('list')} className={`px-3 py-2.5 ${view === 'list' ? 'bg-vermillion/10 text-vermillion' : 'text-ink-muted'}`}><List size={15} /></button>
          </div>
        </div>
        ...
        {loading ? <LoadingSpinner /> : filtered.length === 0 ? (
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
            {filtered.map((book, i) => (
              <motion.div key={book.id} initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: Math.min(i * 0.04, 0.5) }}
                ...
                <div className="aspect-[3/4] bg-parchment-warm flex items-center justify-center">
                  {book.cover_image ? <img src={book.cover_image} alt={book.title} className="w-full h-full object-cover" /> : <BookOpen size={40} className="text-ink-faint/30" />}
                </div>
                <div className="p-4">
                  <span className="text-[10px] font-semibold uppercase tracking-wider text-vermillion">{book.category}</span>
                  <h3 className="font-display text-base font-semibold text-ink mt-1 group-hover:text-vermillion transition-colors">{book.title}</h3>
                  ...
                  <div className="flex items-center justify-between mt-3">
                    <span className="text-ink-faint text-[11px]">{book.total_chapters} {t('chapters', language)}</span>
          ...
          <div className="space-y-3">
            {filtered.map((book, i) => (
              <motion.div key={book.id} initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: Math.min(i * 0.03, 0.4) }}
                ...
                <div className="w-16 h-20 bg-parchment-warm rounded-md flex-shrink-0 flex items-center justify-center">
                  {book.cover_image ? <img src={book.cover_image} alt={book.title} className="w-full h-full object-cover rounded-md" /> : <BookOpen size={24} className="text-ink-faint/30" />}
                </div>
                <div className="flex-1 min-w-0">
                  <span className="text-[10px] font-semibold uppercase tracking-wider text-vermillion">{book.category}</span>
                  <h3 className="font-display text-base font-semibold text-ink group-hover:text-vermillion transition-colors">{book.title}</h3>
                  <p className="text-ink-faint text-sm">{book.author}</p>
                  <p className="text-ink-muted text-[12px] mt-1 line-clamp-2">{book.description}</p>
                </div>
                <div className="flex-shrink-0"><AudioPlayer text={`${book.title} by ${book.author}. ${book.description}`} language={language} compact /></div>
              </motion.div>
            ))}
          </div>
        )}
